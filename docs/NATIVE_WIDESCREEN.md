...
The original ordering table draws the centre. Objects (crates, enemies, fruit) still use that pass; when widescreen is on, `GfxCalcObjectMatrices` keeps its near clip and vertical test but the 4:3 X limits (±316 midpoint, ±256 AABB) are widened to the 16:9 side bands so those meshes are submitted instead of vanishing at the 4:3 edge. HUD and flying pickup icons are not stretched. The side pass enumerates the loaded WGEO meshes independently of the authored SLST visibility list, clips against the near plane, and uses the original materials, UV animation, lighting and fog inputs. It supports normal, fog, water ripple, tint, fog plus tint, and lamp shading. Tint deliberately matches the lighting bank selected by the current recompiled game; changing the native pass alone would create a lighting seam.
