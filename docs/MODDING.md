...
Widescreen is gameplay-only (`titleMap` / `complete` / `cinema` stay 4:3). Texture filters share that gate; dedither applies on all levels. Auto-spin uses the same non-gameplay gate.
